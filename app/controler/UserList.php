...
class UserList extends User
{
    ...
    public function getData($data = null)
    {
        ...
        $pagination = $this->helper::getPagination(self::LIMITE,$pagecurr,'user','UserList');

        $status = (!empty($data['mode']) && $data['mode'] == 1) ? 'actived ' : 'inactived ';
        $modal = '';
        if (!empty($data['status']) && $data['status'] == 'defined') {
            $modal = $this->helper::getModal('confirmChangStatus','User status '.$status.'','This user has '.$status.'.','Ok','front.php?class=UserList','info');
        } else if (!empty($data['status']) && $data['status'] == 'yourself') {
            $modal = $this->helper::getModal('confirmYourself','Your status not change!','It`s not possible change your own state.','Ok','front.php?class=UserList','info');
        }
