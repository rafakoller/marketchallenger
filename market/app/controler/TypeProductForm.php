...
class TypeProductForm extends TypeProduct
{
    ...
    public function getData($data = null)
    {
        ...
        {
            $modal = Helper::getModal('confirmDelete','Do you want Delete?','<h2>Attention!</h2>This type of product wil be deleted.<br><b>Do you confirm this?</b>','Confirm','front.php?class='.self::CLASSLINK.'&status=deleted&key='.$data['key'],'danger');
        }

        // Modal alert do dependency
        if (!empty($data['status']) && $data['status'] == 'havchi')
        {
            $modal = Helper::getModal('alertDependency','Type of product not removed!','This type of product have products associated.<br><b>Remove association before deleting.</b>','Ok','front.php?class='.self::CLASSLINK.'&key='.$data['key'],'info');
        }
