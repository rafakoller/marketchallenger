...
class PurchaseForm extends Purchase
{
    ...
    public function getData($data = null)
    {
        ...
        $modal = (empty($data['status']) || $data['status'] != 'del')?'':Helper::getModal('confirmDelete','Do you want Delete?','<h2>Attention!</h2>This purchase wil be deleted.<br><b>Do you confirm this?</b>','Confirm','front.php?class='.self::CLASSLINK.'&status=deleted&key='.$data['key'],'danger');
        ...
        {
            $modal = Helper::getModal('alertOOStock','Not enough stock!','Cannot remove more than it has.<br><b>Purchase more balance in stock to remove this amount.</b>','Ok','front.php?class='.self::CLASSLINK.'&key='.$data['key'],'info');
        }
